...
import data from '@/app/(crm)/crm/reusable/hardcoded.json';
import {
  type Enquiry,
  type EnquiryStatus,
  type FollowUp,
  ...
  maskPhone,
  getInitials,
} from '@/app/(crm)/crm/reusable/types';
import {
  followUpSchema,
  type FollowUpFormData,
  markLostSchema,
  type MarkLostFormData,
} from '@/app/(crm)/crm/reusable/schema';
...
export default function EnquiryDetailPage({
    ...
    return (
      <div className="crm-page crm-empty-state crm-not-found">
        ...
        <button
          ...
          onClick={() => router.push('/crm/enquiries')}
        >
          <ArrowLeft size={15} />
          Back to Pipeline
        </button>
  ...
  const handleConvert = () => {
    router.push(
      `/students/new?name=${encodeURIComponent(enquiry.name)}&phone=${encodeURIComponent(enquiry.phone)}`
    );
  };
