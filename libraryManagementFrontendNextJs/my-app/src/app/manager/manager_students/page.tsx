...
import { formatCurrency } from '@/app/manager/manager_reusable/format';
...
function ActionsCell({ data }: { data: Student }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, height: '100%' }}>
      <Link href={`/manager/manager_students/${data.id || data.smartId}`} className="mgr-btn-icon" title="View Profile">
        <Eye size={13} />
      </Link>
      <Link href={`/manager/manager_students/${data.id || data.smartId}/edit`} className="mgr-btn-icon" title="Edit">
        <Pencil size={13} />
      </Link>
      {data.due > 0 && (
        <button
    ...
    <div className="mgr-page">
      <div className="mgr-page-header">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <Link href="/manager/manager_students/group" className="mgr-btn-ghost mgr-btn-sm"><Users2 size={14} /> Group Admission</Link>
          <Link href="/manager/manager_students/new" className="mgr-btn-primary"><UserPlus size={14} /> New Admission</Link>
        </div>
      </div>
